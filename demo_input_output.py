...
def show_example_2():
    ...
    print("\n📤 OUTPUT (What the model must produce):\n")
    print(
        "<think>This move creates a fork, attacking both the queen and knight</think>"
    )
...
def show_example_3():
    ...
    print("\n📤 OUTPUT (What the model must produce):\n")
    print(
        "<think>Capturing the rook wins material and simplifies to a winning endgame</think>"
    )
...
def show_stats():
    ...
    for phase, fen in positions:
        ...
        print(
            f"| {phase:10s} | {num_moves:11d} | {'High' if num_moves > 30 else 'Medium' if num_moves > 20 else 'Low':10s} |"
        )
